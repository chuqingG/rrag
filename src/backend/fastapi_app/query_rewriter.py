...
def extract_search_arguments(query_rewrite: str):
    search_query = None
    filters = []
    rewriter = {}
    try:
        rewriter = json.loads(query_rewrite)
    except json.JSONDecodeError as e:
        print(f"Invalid JSON: {e}")
    if "search_query" in rewriter:
        search_query = rewriter["search_query"]
    if "price_filter" in rewriter and rewriter["price_filter"]:
        price_filter = rewriter["price_filter"]
        filters.append(
            {
                "column": "price",
    ...
    if "stars_filter" in rewriter and rewriter["stars_filter"]:
        stars_filter = rewriter["stars_filter"]
        filters.append(
            {
                "column": "stars",
                "operator": stars_filter["operator"],
                "value": stars_filter["value"],
            }
        )
    if "is_best_seller_filter" in rewriter and rewriter['is_best_seller_filter'] and rewriter['is_best_seller_filter'].lower() == 'true':
        filters.append(
            {
